...
function updateBreathStatCallouts(records) {
  ...
  if (selectedJurisdiction) {
    const jSlope = slopePairs.find((s) => s.jurisdiction === selectedJurisdiction);
    if (el("stat-slope")) el("stat-slope").innerHTML = `<h2>In <span class="stat-number">${selectedJurisdiction}</span>: ${jSlope ? fmtCompact(Math.round(jSlope.second)) : "0"} positives/year in the second half</h2><p>${jSlope && jSlope.first ? "Up from " + fmtCompact(Math.round(jSlope.first)) + " in the first half." : "Not enough data for trend."}</p>`;
  } else {
    if (el("stat-slope")) el("stat-slope").innerHTML = `<h2><span class="stat-number">${topSlope ? topSlope.jurisdiction : "—"}</span> has the highest average in the second half</h2><p>With ${topSlope ? fmtCompact(Math.round(topSlope.second)) : "0"} positives per year, up from ${topSlope ? fmtCompact(Math.round(topSlope.first)) : "0"} in the first half.</p>`;
  }
  if (el("stat-indexed")) el("stat-indexed").innerHTML = `<h2>Relative change since ${firstYear}</h2><p>Indexed to ${firstYear} = 100. Lines above 100 show growth, below 100 show decline.</p>`;
}
...
function drawFinesMetrics(records) {
  ...
  if (isSingleMetric) {
    ...
    createChart(canvas, {
      ...
      data: {
        labels: years,
        datasets: [{ data: years.map((y) => byYear[y]), backgroundColor: METRIC_COLORS[metricName] || CHART_COLORS.primary, borderRadius: 4, barThickness: 24 }],
      },
      options: chartDefaults({
  ...
  createChart(canvas, {
    ...
    data: {
      ...
      datasets: [{ data: sorted.map((s) => s[1]), backgroundColor: sorted.map((s) => METRIC_COLORS[s[0]] || CHART_COLORS.primary), borderRadius: 4, barThickness: 24 }],
    },
    options: chartDefaults({
      indexAxis: "y",
...
function drawFinesDetection(records) {
  ...
  const methodColors = { "Fixed camera": "#38bdf8", "Mobile camera": "#818cf8", "Police issued": "#fb923c", "Red light camera": "#34d399", "Fixed or mobile camera": "#a78bfa", "Average speed camera": "#fbbf24" };
  const metricTotals = {};
  ...
  app.innerHTML = `
    <div class="dataset-page">
      ...
      <div class="dataset-content">
        ...
        <div class="chart-section" id="slope">
          <div class="stat-callout" id="stat-slope"></div>
          <h3>How positives shifted between halves</h3>
        ...
        <div class="chart-section" id="indexed">
          ...
          <p class="chart-desc">Indexed to the first year = 100, showing relative change.</p>
    ...
    <div class="dataset-page">
      ...
      <div class="dataset-content">
        ...
        <div class="chart-section" id="substances">
          ...
          <p class="chart-desc">Substance detection counts across all jurisdictions.</p>
        ...
        <div class="chart-section" id="composition">
          ...
          <p class="chart-desc">How the mix of detected substances has shifted.</p>
...
function drawDrugSubstances(records) {
  const byYear = {};
  records.forEach((r) => {
    if (!byYear[r.series_year]) byYear[r.series_year] = { cannabis: 0, amphetamine: 0, methylamphetamine: 0, ecstasy: 0, cocaine: 0, other: 0 };
  ...
  const years = getActiveYears(byYear, ["cannabis", "amphetamine", "methylamphetamine", "ecstasy", "cocaine", "other"]);
  ...
  createChart(canvas, {
    ...
    data: {
      ...
      datasets: substances.map((s) => ({
        ...
        backgroundColor: years.length <= 3 ? SUBSTANCE_COLORS[s] + "cc" : "transparent",
        tension: 0.3,
        pointRadius: 4,
        pointHoverRadius: 7,
        borderWidth: 2.5,
        borderRadius: years.length <= 3 ? 4 : 0,
        barThickness: years.length <= 3 ? 34 : undefined,
      })),
    },
    options: chartDefaults({
      scales: { ...chartDefaults().scales, y: { ...chartDefaults().scales.y, ticks: { ...chartDefaults().scales.y.ticks, callback: (v) => fmtCompact(v) } } },
